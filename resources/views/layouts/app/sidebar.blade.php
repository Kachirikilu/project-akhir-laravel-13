                ...
                @foreach ($navItems as $item)
                    ...
                    @elseif($item['type'] === 'dropdown-kelas')
                            ...
                            if ($lastKelas) {
                                $subMenus[] = [
                                    ...
                                    'active' =>
                                        ...
                                        request()->route('kode') === $lastKelas['kode'],
                                ];
                            }

                            if ($lastSesi) {
                                $subMenus[] = [
                                    'label' => $lastSesi['kode'] . '-' . $lastSesi['kode_jadwal'],
                                    'url' => $lastSesi['url'],
                                    ...
                                    'active' =>
                                        ...
                                        request()->route('kode') === $lastSesi['kode'] &&
                                        request()->route('kode_jadwal') === $lastSesi['kode_jadwal'],
                                ];
                            }
                        ...
                        <div class="relative mr-2">
                                ...
                                <flux:menu x-data="{
                                    ...
                                    @foreach ($subMenus as $sub)
                                        <flux:menu.item :href="$sub['url']" wire:navigate
                                            ...
                                            <span
                                                :class="((
                                                        '{{ $sub['param'] }}' === 'kelas-management' && currentRoute === 'kelas-management'
                                                    ) || (
                                                        '{{ $sub['param'] }}' === 'jadwal-management' && currentRoute === 'jadwal-management' && currentKode === '{{ $lastKelas['kode'] ?? '' }}'
                                                    ) || (
                                                        '{{ $sub['param'] }}' === 'sesi-management' && currentRoute === 'sesi-management' && currentKode === '{{ $lastSesi['kode'] ?? '' }}' && currentJadwal === '{{ $lastSesi['kode_jadwal'] ?? '' }}'
                                                    )) ?
                                                'bg-[var(--main-table-color)] dark:bg-white/10 text-[var(--contrast-main-text)] font-semibold border-l-2 border-[var(--border-main-color)] shadow-sm' :
                                                'text-[var(--contrast-main-text)] group-hover:text-[var(--contrast-third-text)] group-hover:bg-[var(--main-pop-up-color)] dark:group-hover:bg-white/5'"
                                                
                                                class="pr-7 flex items-center rounded-md w-full h-full text-xs px-3 py-1.5 transition-all duration-300 ease-in-out min-w-0 [margin-left:{{ ($sub['level'] ?? 0) * 12 }}px]">
                            ...
                            {{-- Tombol utama (persis seperti OBE) --}}
                            <button type="button"
                            ...
                            {{-- Submenu besar (persis OBE style) --}}
                            <div x-show="expanded && openKelasMenu" x-cloak
                                ...
                                @foreach ($subMenus as $sub)
                                    <a href="{{ $sub['url'] }}" wire:navigate
                                        style="margin-left: {{ $sub['level'] * 12 }}px" @class([
