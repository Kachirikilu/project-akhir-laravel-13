                ...
                @foreach ($navItems as $item)
                    ...
                    @elseif ($item['type'] === 'dropdown')
                        ...
                        <div class="relative mx-1">
                            {{-- TOMBOL TUNGGAL (DOM tetap ringan karena tidak ada duplikasi elemen) --}}
<button type="button"
    @click="expanded ? (openObeMenu = !openObeMenu) : $refs.fluxDropdown.click()"
    :class="expanded
        ? 'w-full justify-between p-2'
        : 'w-8 h-8 justify-center mx-auto'"
    class="cursor-pointer flex items-center text-xs rounded-lg transition-colors duration-300
    {{ $isObeActive
    ...
    <div class="flex items-center overflow-hidden">
        ...
        {{-- Animasi sama persis seperti mode link --}}
        <span
            x-show="expanded"
            x-cloak
            ...
            x-transition:leave-start="opacity-100 translate-x-0"
            x-transition:leave-end="opacity-0 translate-x-4"
            class="ml-3 whitespace-nowrap overflow-hidden text-ellipsis block"
        >
            {{ $item['label'] }}
        </span>
    </div>
    ...
    {{-- trigger dropdown jangan diubah --}}
    <div x-show="!expanded" class="absolute inset-0 opacity-0 pointer-events-none"
>
        <flux:dropdown position="right" align="start">
            {{-- penting: tetap pakai x-ref di sini --}}
            <div
                x-ref="fluxDropdown"
            ...
            <flux:menu
                class="w-44 bg-neutral-800 border border-neutral-700/50 shadow-xl p-1">
                ...
                @foreach ($subMenus as $sub)
                    ...
                    <flux:menu.item
                        ...
                        {{ $isActive
                            ? 'bg-amber-500/20 text-amber-400 font-semibold border-l-2 border-amber-500 pl-2'
                ...
                @endforeach
                            ...
                            {{-- COLLAPSIBLE LIST MODE: List bertingkat kebawah saat sidebar lebar --}}
