            ...
            <nav class="flex-1 space-y-1 no-scrollbar" x-data="{ openObeMenu: {{ request()->routeIs('rps-management') ? 'true' : 'false' }} }">
                ...
                @foreach ($navItems as $item)
                    ...
                    @elseif ($item['type'] === 'dropdown')
                        ...
                        {{-- MODE 1: SIDEBAR LEBAR (Collapsible List biasa) --}}
                        <div x-show="expanded" x-transition:enter="transition-all duration-300 ease-out"
                            x-transition:leave="transition-all duration-200 ease-in" class="relative mx-1">
                            <button type="button" @click="openObeMenu = !openObeMenu"
                                class="cursor-pointer w-full flex items-center justify-between text-xs p-2 rounded-lg transition-colors {{ $isObeActive ? 'bg-white/10 text-[var(--main-text)]' : 'text-[var(--main-text)]/80 hover:bg-white/10 hover:text-[var(--main-text)]' }}">

                                <div class="flex items-center overflow-hidden">
                                    <flux:icon :name="$item['icon']" variant="outline" class="w-4 h-4 shrink-0" />

                                    {{-- Animasi meluncur dan menghilang bersih ke kiri --}}
                            ...
                            <div x-show="openObeMenu" x-cloak class="mt-1 space-y-1 pl-4 w-full">
                        ...
                        {{-- MODE 2: SIDEBAR MENGECIL (Pop-up Flux Dropdown) --}}
                        <div x-show="!expanded" x-transition:enter="transition-all duration-300 ease-out delay-300"
                            x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100"
                            x-transition:leave="transition-all duration-150 ease-in" class="mx-1 flex justify-center">

                            <flux:dropdown position="right" align="start">
                                <button type="button"
                                    class="cursor-pointer w-8 h-8 flex items-center justify-center rounded-lg transition-colors hover:bg-white/10 text-[var(--main-text)]/80 hover:text-[var(--main-text)] {{ $isObeActive ? 'bg-white/20 text-[var(--main-text)]' : '' }}"
                                    title="{{ $item['label'] }}">
                                    <flux:icon :name="$item['icon']" variant="outline" class="w-4 h-4 shrink-0" />
                                </button>
