<flux:modal name="user-excel-modal" wire:model.live="showUserExcelModal" flyout wire:key="user-excel-modal-flyout" 
    @refresh-data-user.window="if (!$wire.showUserExcelModal) $store.user.reset()"
    class="w-full md:w-screen-2xl max-w-screen-2xl max-h-[98vh] !p-4 sm:!p-6 md:!p-8 !bg-[var(--second-pop-up-color)] !text-[var(--contrast-main-text)] text-xs sm:text-sm scrollbar-large">
    ...
    <div wire:loading wire:target="saveUserExcel">
        <div
            ...
            <p wire:stream="import-progress" class="mt-4 text-sm font-medium text-[var(--contrast-second-text)] italic">Menyinkronkan...</p>
        </div>
    </div>
