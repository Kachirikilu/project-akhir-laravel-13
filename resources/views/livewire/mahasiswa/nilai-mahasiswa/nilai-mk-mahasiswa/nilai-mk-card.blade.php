<div wire:key="view-card-nilai-mk">
    ...
    <x-global.main-layout-card :paginator="$nilais">
        ...
        @forelse($nilais as $n)
            <div wire:key="nilai-rps-{{ $n->id }}"
                ...
                <div
                    class="pt-3 border-t table-border/40 -mx-5 -mb-5 p-4 bg-[var(--second-table-trans)] rounded-b-xl flex items-center justify-between">
                    <p class="text-xs font-medium text-[var(--contrast-third-text)] flex items-center gap-1.5">
                        ...
                        Beban Kuliah: <strong
                            class="text-[var(--contrast-main-text)]">{{ $n->sks ?? ($n->rps_rel?->sks ?? 0) }}
                            SKS</strong>
                    </p>
