...
<button type="button" x-show="{{ $xShow }}" x-transition:enter="transition ease-out duration-200"
    ...
    @click="
        ...
        @if ($xAlpine ?? null)
            if ($store.{{ $alpine ?? 'config' }}) {
                Alpine.raw($store.{{ $alpine ?? 'config' }});
                let store = $store.{{ $alpine ?? 'config' }};
                let path = '{{ $xAlpine }}'.replace(/\[(\d+)\]/g, '.$1').split('.');
                let last = path.pop();
                let target = path.reduce((obj, key) => obj[key] = obj[key] || {}, store);
                target[last] = '';
            }
        @endif
        ...
        @if ($xAlpine2 ?? null)
            if ($store.{{ $alpine ?? 'config' }}) {
                let store = $store.{{ $alpine ?? 'config' }};
                let path = '{{ $xAlpine2 }}'.replace(/\[(\d+)\]/g, '.$1').split('.');
                let last = path.pop();
                let target = path.reduce((obj, key) => obj[key] = obj[key] || {}, store);
                target[last] = '';
            }
