<div class="flex items-start gap-3">
    <div class="mt-1.5 h-1.5 w-1.5 rounded-full bg-[var(--focus-color)] shrink-0"></div>
    <p class="text-xs sm:text-sm text-[var(--contrast-second-text)] leading-relaxed">
        Pastikan semua kolom <strong class="text-[var(--contrast-main-text)] font-semibold">wajib diisi</strong> dengan
        benar.
    </p>
</div>

<div class="flex items-start gap-3">
    <div class="mt-1.5 h-1.5 w-1.5 rounded-full bg-[var(--focus-color)] shrink-0"></div>
    <p class="text-xs text-sm text-[var(--contrast-second-text)] leading-relaxed">
        Perubahan akan tersimpan segera setelah formulir dikirim.
    </p>
</div>
