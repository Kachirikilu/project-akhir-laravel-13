...
@php
        // 1. Deteksi keaktifan menu induk secara global
        $isProdiActive =
            request()->routeIs('program-studi-management') ||
            request()->routeIs('capaian-management') ||
            request()->routeIs('rps-capaian-management') ||
            collect(['program-studi-management', 'capaian-management', 'rps-capaian-management'])->some(fn($r) => str_contains(request()->url(), $r));
        ...
        // 2. Set parameter URL 'switchTable' => '' agar URL bersih saat diklik
        $subMenus = [
            [
                'label' => 'Program Studi',
                ...
                'active' => false, // Kita pasang false karena penentu aktifnya akan diambil alih Alpine.js di bawah
        ...
        foreach ($prodiHistory as $prodi) {
            ...
            $subMenus[] = [
                ...
                'level' => 1,
                'active' => request()->routeIs('capaian-management') && request()->route('kode_pr') === $kodePr,
                'active-sub' => request()->routeIs('rps-capaian-management') && request()->route('kode_pr') === $kodePr,
            ];
            ...
            if (isset($groupedCpl[$kodePr])) {
                foreach ($groupedCpl[$kodePr] as $cpl) {
                    $subMenus[] = [
                        ...
                        'level' => 2,
                        'active' => request()->routeIs('rps-capaian-management') && request()->route('kode_pr') === $kodePr && request()->route('kode_cpl') === $cpl['kode_cpl'],
                    ];
                }
            }
        }
        ...
        $subMenus[] = [
            'label' => 'Departemen',
            'url' => route('program-studi-management', ['switchTable' => 'departemen']),
            'param' => 'departemen',
            'icon' => 'book-open',
            'color' => 'text-amber-600 dark:text-amber-400',
            'active' => false, // Diambil alih Alpine
        ];

        $subMenus[] = [
            'label' => 'Fakultas',
            'url' => route('program-studi-management', ['switchTable' => 'fakultas']),
            'param' => 'fakultas',
