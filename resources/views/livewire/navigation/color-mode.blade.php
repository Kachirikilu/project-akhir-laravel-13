{{-- x-data cukup kosong saja, karena kita menggunakan $store --}}
<div x-data class="flex flex-col items-center gap-1 mb-6">
    
    {{-- Container Tema --}}
    <div x-show="typeof expanded === 'undefined' ? true : expanded" 
        ...
        <template x-for="theme in $store.theme_manager.allThemes" :key="theme.id">
            <button type="button" 
                ...
                <span x-show="$store.theme_manager.currentTheme === theme.id" class="absolute inset-0 flex items-center justify-center">
    ...
    <div x-show="typeof expanded === 'undefined' ? true : expanded" 
        ...
        <button @click="$store.theme_manager.toggleAutoPlay()" type="button" 
            ...
            <div class="w-1.5 h-1.5 rounded-full transition-all duration-500 opacity-50 group-hover:opacity-100"
                :style="`background-color: ${$store.theme_manager.getThemeColor(1)}`"
                :class="$store.theme_manager.isAutoPlaying && 'animate-bounce [animation-delay:0.2s] translate-y-[1px]'"></div>
        </button>
