<div x-data="{
    ...
    get visibleCards() {
        return Array
            .from(this.$root.querySelectorAll('.card-sesi-item'))
            .filter(el => {
                try {
    ...
    sortCards() {
        ...
        let direction = this.$store.sesi.sortDirection === 'desc' ?
            -1 :
            1
            ...
            <div x-show="totalFilteredItems > perPage"
                ...
                <div class="text-xs text-[var(--contrast-second-text)]">
                    ...
                    <span class="font-semibold text-[var(--contrast-main-text)]"
                        x-text="Math.min((($store.sesi.currentPage - 1) * perPage) + 1, totalFilteredItems)">
                    </span>

                    sampai

                    <span class="font-semibold text-[var(--contrast-main-text)]"
                        x-text="Math.min($store.sesi.currentPage * perPage, totalFilteredItems)">
                    </span>

                    dari

                    <span class="font-semibold text-[var(--contrast-main-text)]" x-text="totalFilteredItems">
                    </span>
                ...
                <div class="flex items-center gap-1">
                    ...
                    <div class="flex items-center gap-1 mx-2 text-xs text-[var(--contrast-second-text)]">
                        ...
                        <span class="font-semibold text-[var(--contrast-main-text)]" x-text="totalPages"></span>
                    </div>
                    <button type="button"
                        @click="
                    if ($store.sesi.currentPage < totalPages) {
                        $store.sesi.currentPage++;
                        }
                    "
                        :disabled="$store.sesi.currentPage >= totalPages"
                        class="px-3 py-1 text-xs font-medium rounded-lg border border-[var(--border-table-color)] bg-[var(--main-table-trans)] disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed">
                        Selanjutnya
                    </button>
                </div>
