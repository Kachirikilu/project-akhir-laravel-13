<div x-data="{
    get currentPage() {
        return this.$store.sesi.currentPage || 1
    },

    set currentPage(val) {
        this.$store.sesi.currentPage = val
    },

    get perPage() {
        return this.$store.sesi.perPage || 8
    },

    get visibleCards() {
        return Array
            .from(document.querySelectorAll('.card-sesi-item'))
            .filter(el => {
                try {
    ...
    sortCards() {
        ...
        let direction =
            this.$store.sesi.sortDirection === 'desc' ?
            -1 :
            1
        ...
        let container =
            this.$root.querySelector('.card-sesi-item')
            ?.parentElement

        if (!container) return

        let cards = Array.from(
            container.querySelectorAll('.card-sesi-item')
        )
        ...
        // hitung index global hasil sort
        let visibleIndex = 0

        cards.forEach(card => {
            try {
                const passed = Alpine.evaluate(
                    card,
                    'matchSearchPassed'
                )

                card.dataset.visibleIndex =
                    passed ?
                    visibleIndex++
                    :
                    -1
            } catch {
                card.dataset.visibleIndex = -1
            }
        })
    },
    ...
    init() {
        if (!this.$store.sesi.currentPage) {
            this.$store.sesi.currentPage = 1
        }
    ...
    <x-global.main-layout-card :paginator="$sesis">
        ...
        @foreach ($sesis as $index => $s)
            ...
            <div wire:key="kelas-{{ $s->id }}" x-data="{
                ...
                get localVisibleIndex() {
                    return Number(
                        this.$el.dataset.visibleIndex ?? -1
                    );
                }
            }"
                ...
                <div class="flex flex-col flex-1 justify-between mt-3 text-sm">
                    ...
                    <div x-show="expanded" x-collapse class="mt-2 transition-all duration-300">
                        <div class="grid grid-cols-1 {{ $isUjian ? 'sm:grid-cols-3' : '' }} gap-2">
                            <div @click.stop
                                ...
                                <div class="flex items-center justify-between gap-2">
                                    <span
                                        ...
                                        & Bobot</span>
                                    <span
                                        class="text-xs px-2 py-1 rounded-lg font-semibold bg-[var(--main-table-color)] border border-[var(--border-table-color)]">Bobot:
                            ...
                            <div
                                class="sm:col-span-1 p-3 rounded-xl bg-[var(--second-table-trans)] border border-[var(--border-table-color)]/30">
                                <span
        ...
        <div x-show="$store.sesi.totalFilteredItems === 0" x-transition.opacity
            class="col-span-6 text-center p-12 rounded-xl border border-dashed border-[var(--border-table-color)] bg-[var(--main-table-trans)]">
        ...
        <x-slot:footer>
            <div x-show="$store.sesi.totalFilteredItems > $store.sesi.perPage"
                ...
                <div class="text-xs text-[var(--contrast-second-text)]">
                    ...
                    <span class="font-semibold text-[var(--contrast-main-text)]"
                        x-text="Math.min((($store.sesi.currentPage - 1) * $store.sesi.perPage) + 1, $store.sesi.totalFilteredItems)">
                    </span>

                    sampai

                    <span class="font-semibold text-[var(--contrast-main-text)]"
                        x-text="Math.min($store.sesi.currentPage * $store.sesi.perPage, $store.sesi.totalFilteredItems)">
                    </span>

                    dari

                    <span class="font-semibold text-[var(--contrast-main-text)]"
                        x-text="$store.sesi.totalFilteredItems">
                    </span>
                ...
                <div class="flex items-center gap-1">
                    ...
                    <button type="button"
                        @click="
                    if ($store.sesi.currentPage > 1) {
                        $store.sesi.currentPage--;
                    }
                "
                        :disabled="$store.sesi.currentPage <= 1"
                        class="px-3 py-1 text-xs font-medium rounded-lg border border-[var(--border-table-color)] bg-[var(--main-table-trans)] disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed">
                        Sebelumnya
                    </button>
                    ...
                    <div class="flex items-center gap-1 mx-2 text-xs text-[var(--contrast-second-text)]">
                        ...
                        <span class="font-semibold text-[var(--contrast-main-text)]" x-text="$store.sesi.currentPage">
                        </span>

                        dari

                        <span class="font-semibold text-[var(--contrast-main-text)]" x-text="$store.sesi.totalPages">
                        </span>
                    </div>

                    <button type="button"
                        @click="
                    if ($store.sesi.currentPage < $store.sesi.totalPages) {
                        $store.sesi.currentPage++;
                    }
                "
