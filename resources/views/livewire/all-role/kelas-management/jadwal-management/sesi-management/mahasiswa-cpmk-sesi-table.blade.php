<div wire:key="view-table-mahasiswa-cpmk-sesi">
    ...
    <x-global.main-layout-table :paginator="$users" :onlyAdmin="!Auth::user()->admin">
        ...
        <x-slot:header>
            <tr>
                ...
                @if (Auth::user()->admin || Auth::user()->dosen)
                    <th colspan="{{ $showMore ? 3 : 2 }}" class="table-head-sub">
                        Nilai Mahasiswa
                    </th>
                    ...
                    @foreach ($groupsCpmk as $kodeCpmk => $pertemuans)
                        @php
                            ...
                            $colorClass = $hasEvaluasi ? 'font-bold text-[var(--focus-color-special)]' : '';
                        @endphp

                        <th colspan="{{ $showMore ? 3 : 2 }}" x-data="{ showLine: {{ $hasEvaluasi ? 'true' : 'false' }} }"
                            ...
                            <div class="absolute bottom-0 left-1/2 -translate-x-1/2 w-[90%] h-[3px] mb-1 origin-center"
                                style="background: var(--focus-color-special);" x-show="showLine"
