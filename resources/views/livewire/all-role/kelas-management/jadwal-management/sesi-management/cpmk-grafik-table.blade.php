<div class="rps-pdf bg-white">
    @include('livewire.all-role.kelas-management.jadwal-management.sesi-management.cpmk-grafik-show.cpmk-grafik-data')
    ...
    <div class="w-full bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800 shadow-sm overflow-hidden">
        ...
        <div class="w-full min-h-90 overflow-x-auto p-4 scrollbar-large"
            wire:key="chart-{{ $search ?? '' }}-{{ $perPage ?? '' }}-{{ $sortField ?? '' }}-{{ $sortDirection ?? '' }}-{{ $switchTable ?? '' }}">
