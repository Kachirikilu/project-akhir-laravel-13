...
<div x-data="{
    ...
    get filteredAndSortedIds() {
        ...
        let filtered = this.rawItems.filter(item => {
            ...
            if (item.kode_scpmk_raw.includes(query) || (cleanQuery && item.searchKodeSCPMK.includes(cleanQuery))) {
                return true;
            }
            ...
            let cocokBobot = item.searchBobot.some(bText => bText === query || bText === dotQuery || bText.includes(query) || bText.includes(dotQuery));
