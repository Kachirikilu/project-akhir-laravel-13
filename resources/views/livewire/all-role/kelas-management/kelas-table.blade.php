<div wire:key="view-table-kelas">

    <x-global.main-layout-table wire:key="view-table-kelas" :paginator="$kelas">
        ...
        @forelse($kelas as $k)
            <tr wire:key="kelas-{{ $k->id }}" data-kelas-id="{{ $k->id }}"
                ...
                <td class="table-second table-border-x text-center whitespace-nowrap">
                    @if (!$k->trashed())
                        <x-button-action color="emerald" href="{{ route('jadwal-management', $k->kode, 'card') }}"
                            wire:navigate>
