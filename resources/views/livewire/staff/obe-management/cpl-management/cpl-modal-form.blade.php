        ...
        <h3 class="text-xl font-semibold">
            ...
            <template x-if="$store.cpl?.typeModal == '1'" x-cloak>
                <flux:badge icon="academic-cap" color="sky" size="lg">
                    <span x-text="$store.cpl?.isEdit ? 'Edit CPL - Program Studi' : 'Tambah CPL - Program Studi'"></span>
                </flux:badge>
            </template>

            <template x-if="$store.cpl?.typeModal == 2" x-cloak>
                <flux:badge icon="book-open" color="sky" size="lg">
                    <span x-text="$store.cpl?.isEdit ? 'Edit CPL - Departemen' : 'Tambah CPL - Departemen'"></span>
                </flux:badge>
            </template>

            <template x-if="$store.cpl?.typeModal == 3" x-cloak>
                <flux:badge icon="building-library" color="sky" size="lg">
                    <span x-text="$store.cpl?.isEdit ? 'Edit CPL - Fakultas' : 'Tambah CPL - Fakultas'"></span>
                </flux:badge>
            </template>

            <template x-if="$store.cpl?.typeModal == 4" x-cloak>
                <flux:badge icon="globe-alt" color="sky" size="lg">
                    <span x-text="$store.cpl?.isEdit ? 'Edit CPL - Universitas' : 'Tambah CPL - Universitas'"></span>
                </flux:badge>
            </template>
