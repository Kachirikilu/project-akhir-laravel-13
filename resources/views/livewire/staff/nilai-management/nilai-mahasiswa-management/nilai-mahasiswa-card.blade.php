        ...
        @foreach ($periodes as $index => $p)
            ...
            <template x-if="itemVisibilityMap[{{ $currentId }}]?.visible">
                <div wire:key="kelas-periode-card-{{ $currentId }}"
                    ...
                    class="flex flex-col rounded-[20px] overflow-hidden border border-[var(--border-table-color)] bg-[var(--main-table-trans)]/50 transition-all duration-200 hover:shadow-lg cursor-pointer">
                    ...
                    <div class="flex flex-1 flex-col gap-2.5 p-4">
                        <div
                            ...
                            <span
                                class="text-[10px] font-bold uppercase tracking-[0.06em] text-[var(--contrast-third-text)]">
                                Total SKS: {{ $p->total_sks }} SKS
                            </span>
                        </div>
                    ...
                    <div class="px-4 pb-4 flex items-center gap-1.5">
                        <button
                            ...
                            href="{{ route('rps-mahasiswa-management', ['nim' => $this->nim, 'ganjil_genap' => $p->ganjil_genap, 'akademik' => str_replace('/', '-', $p->akademik)]) }}"
                            wire:navigate>
