...
<body class="min-h-screen antialiased bg-[var(--wadah-color)] dark:bg-neutral-950 text-[var(--contrast-main-text)] scrollbar-large">
        ...
        <div class="flex items-center gap-2">
            ...
            @auth
                <a href="{{ route('dashboard') }}" wire:navigate
                    class="px-4 py-2 rounded-xl text-xs font-bold bg-[var(--main-color)] text-white flex items-center gap-2 transition hover:opacity-90 active:opacity-80">
            ...
            @else
                @if (\App\Models\User::count() === 0)
                    <a href="{{ route('register') }}" wire:navigate
                        class="px-4 py-2 rounded-xl text-xs font-bold bg-[var(--main-color)] text-white shadow-lg">
                        Daftar Admin
                    </a>
                @else
    ...
    <header class="py-24 px-5 text-center space-y-8">
        ...
        <p class="text-[var(--contrast-third-text)] max-w-lg mx-auto text-lg leading-relaxed">
            Platform modern untuk pengelolaan RPS, kurikulum, dan capaian pembelajaran yang terpusat.
        </p>
    </header>
    ...
    <section class="relative z-10 bg-[var(--sub-table-color)]/50 border-t border-[var(--border-table-color)] px-5 sm:px-8 py-12">
        <div class="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14">

            {{-- Fitur --}}
            <div class="flex flex-col gap-5">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--contrast-third-text)] mb-1">Fitur Utama</p>
                    <h2 class="text-lg sm:text-xl font-bold tracking-tight text-[var(--contrast-main-text)]">Semua yang Anda butuhkan</h2>
                </div>

                <div class="grid grid-cols-2 gap-3">
                    @foreach ([
                        ['icon' => 'document-text', 'title' => 'Manajemen RPS', 'desc' => 'CPL, CPMK, Sub-CPMK sesuai standar OBE.', 'color' => 'var(--focus-color)', 'soft' => 'color-mix(in srgb, var(--focus-color) 12%, transparent)'],
                        ['icon' => 'users', 'title' => 'Kelas & Absensi', 'desc' => 'Jadwal dan presensi mahasiswa secara real-time.', 'color' => '#10b981', 'soft' => 'rgba(16,185,129,0.12)'],
                        ['icon' => 'chart-bar', 'title' => 'Capaian Lulusan', 'desc' => 'IPK, IPS, dan analitik nilai per Program Studi.', 'color' => '#f59e0b', 'soft' => 'rgba(245,158,11,0.12)'],
                        ['icon' => 'building-library', 'title' => 'Akreditasi Prodi', 'desc' => 'Pantau kinerja dan status akreditasi terpusat.', 'color' => '#7c3aed', 'soft' => 'rgba(124,58,237,0.12)'],
                    ] as $feat)
                        <div class="flex flex-col gap-3 rounded-[14px] border border-[var(--border-table-color)] bg-[var(--second-table-color)] p-4 hover:shadow-sm transition-shadow">
                            <div class="flex h-9 w-9 items-center justify-center rounded-[9px]"
                                style="background: {{ $feat['soft'] }};">
                                <flux:icon name="{{ $feat['icon'] }}" class="w-4 h-4" style="color: {{ $feat['color'] }};" />
                            </div>
                            <div>
                                <p class="text-xs sm:text-sm font-bold text-[var(--contrast-main-text)] mb-0.5">{{ $feat['title'] }}</p>
                                <p class="text-[11px] sm:text-xs text-[var(--contrast-third-text)] leading-relaxed">{{ $feat['desc'] }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="flex flex-col gap-5">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--contrast-third-text)] mb-1">Akses Pengguna</p>
                    <h2 class="text-lg sm:text-xl font-bold tracking-tight text-[var(--contrast-main-text)]">Tiga peran, satu platform</h2>
                </div>
                ...
                <div class="flex flex-col gap-3">
                    @foreach ([
                        ...
                        ['tag' => 'Dosen', 'name' => 'Dosen Pengampu', 'desc' => 'Kelola RPS, Kelas, Pertemuan, dan Penilaian dalam lingkup Mata Kuliah yang diampu.', 'accent' => '#10b981', 'soft' => 'rgba(16,185,129,0.12)', 'icon' => 'presentation-chart-bar'],
                        ['tag' => 'Mahasiswa', 'name' => 'Mahasiswa', 'desc' => 'Lihat Jadwal, Absensi, Nilai, dan Progres Capaian Pembelajaran Semester berjalan.', 'accent' => '#06b6d4', 'soft' => 'rgba(6,182,212,0.12)', 'icon' => 'academic-cap'],
                    ] as $role)
                        <div class="flex items-start gap-4 rounded-[14px] border border-[var(--border-table-color)] bg-[var(--second-table-color)] p-4 hover:shadow-sm transition-shadow">
                            <div class="flex h-10 w-10 items-center justify-center rounded-[10px] flex-shrink-0"
                                style="background: {{ $role['soft'] }};">
                                <flux:icon name="{{ $role['icon'] }}" class="w-4.5 h-4.5" style="color: {{ $role['accent'] }};" />
                            </div>
                            <div class="flex flex-col gap-1 min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <span class="text-xs sm:text-sm font-bold text-[var(--contrast-main-text)]">{{ $role['name'] }}</span>
                                    <span class="text-[9px] font-bold uppercase tracking-[0.07em] rounded-md px-2 py-0.5"
                                        style="background: {{ $role['soft'] }}; color: {{ $role['accent'] }};">
                                        {{ $role['tag'] }}
                                    </span>
                                </div>
                                <p class="text-[11px] sm:text-xs text-[var(--contrast-third-text)] leading-relaxed">{{ $role['desc'] }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
