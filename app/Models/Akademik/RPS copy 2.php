...
class RPS extends Model
{
    ...
    {
        return $this->belongsToMany(TimDosen::class, 'rps_pivot_tim_dosen', 'rps_id', 'tim_dosen_id')
            ->orderBy('sort_order')
            ->withTimestamps();
    }
    ...
    protected function scpmkAtr(): Attribute
    {
        return Attribute::get(function () {
            ...
            $tim = $this->tim_dosens->first();

            $dosenRps = $tim?->dosens ?? collect();
        ...
        // Ambil tim pertama yang dipakai RPS
        $tim = $this->tim_dosens->first();

        // Semua dosen dalam tim
        $dosenRps = $tim?->dosens ?? collect();

// Ambil dosen yang mengajar pada pertemuan ini
if ($item instanceof \App\Models\Akademik\SubCPMK && ! empty($item->materi)) {

    $item->dosens_collection = $dosenRps
        ->filter(function ($dosen) use ($p) {

            $pertemuan = $dosen->pivot->pertemuan_ke ?? null;

            // null / kosong = berlaku semua pertemuan
            if (blank($pertemuan)) {
                return true;
            }

            // Sudah array
            if (is_array($pertemuan)) {
                return in_array($p, $pertemuan);
            }

            // Collection
            if ($pertemuan instanceof \Illuminate\Support\Collection) {
                return $pertemuan->contains($p);
            }

            // JSON string
            if (is_string($pertemuan)) {

                $decoded = json_decode($pertemuan, true);

                if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                    return in_array($p, $decoded);
                }

                // Format "1,2,3"
                $decoded = collect(explode(',', $pertemuan))
                    ->map(fn ($v) => (int) trim($v))
                    ->filter()
                    ->values()
                    ->all();
    ...
    $item->dosens_collection = collect();
        ...
        // Copy atribut pivot ke object agar Blade tetap mudah dipakai
        $item->dosens_collection->transform(function ($dosen) {

            $dosen->is_ketua = (bool) ($dosen->pivot->is_ketua ?? false);
            $dosen->peran = $dosen->pivot->peran ?? 'Pengajar';
            $dosen->pertemuan_ke = $dosen->pivot->pertemuan_ke;

            return $dosen;
        });

        // Jika semua dosen tim muncul, kosongkan collection
        $currentIds = $item->dosens_collection
            ->pluck('id')
            ->sort()
            ->values()
            ->toArray();
        ...
        $item->dosen_id_string = $item->dosens_collection
            ->pluck('id')
            ->sort()
            ->implode(',');
