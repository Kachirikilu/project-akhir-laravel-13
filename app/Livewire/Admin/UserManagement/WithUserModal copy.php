...
trait WithUserModal
{
    ...
    public $mahasiswa_rps_items_list = [];

    public $mahasiswa_rps_modal_page = 3;

    public $mahasiswa_rps_id;

    protected $mahasiswa_rps_modal_paginator;
    ...
    public function editUser($id, $withRPS = false, $isRPS = false)
    {
        ...
        try {
            ...
            if ($withRPS) {
                ...
                if ($user->mahasiswa) {
                    $this->mahasiswa_rps_id = $user->mahasiswa->id;
                    $this->resetPage('mahasiswa_rps_modal_page');
    ...
    {
        if (empty($this->mahasiswa_rps_id)) {
            return;
        }

        $mahasiswa = Mahasiswa::find($this->mahasiswa_rps_id);
        ...
        $rps = RPS::query()
            ...
            ->paginate(
                $this->mahasiswa_rps_modal_page,
                ['*'],
                'mahasiswa_rps_modal_page'
            );

        $this->mahasiswa_rps_items_list = $this->mapRPS($rps);
        $this->mahasiswa_rps_modal_paginator = $rps;
    }
