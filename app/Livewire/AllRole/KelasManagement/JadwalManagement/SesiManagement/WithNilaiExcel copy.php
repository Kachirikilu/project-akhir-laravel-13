...
trait WithNilaiExcel
{
    ...
    public function updatedParsedNilaiRows(array $value): void
    {
        if (! is_array($value)) {
            return;
        }
        ...
        foreach ($value as $rowIndex => $row) {
            ...
            $value[$rowIndex]['nilai_angka'] = $average;
            $value[$rowIndex]['nilai_index'] = $gradeInfo['nilai_index'];
            $value[$rowIndex]['nilai_mutu'] = $gradeInfo['nilai_mutu'];
        }

        $this->parsedNilaiRows = $value;
    }
    ...
    public function recalculateRowNilai(int $rowIndex)
    {
        ...
        $this->parsedNilaiRows[$rowIndex] = $row;
    }
    ...
    {
        // dump(spl_object_id((object)$validated));
        // dump($validated['sub_cpmk'][0]['nilai']);
