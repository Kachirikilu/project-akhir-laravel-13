...
trait RekapCapaian
{
    ...
    public function generateRekapCapaian($prId = null, $cooldown = null)
    {
        ...
        if (Cache::has($runningAllKey) || $isAllProdiRunningInDB) {
            $this->toast(
                text: 'Gagal! Proses Rekap Capaian untuk SELURUH PRODI sedang berjalan di latar belakang. Silakan tunggu hingga selesai.',
                variant: 'warning',
                duration: 12000
            );
        ...
        if ($prId === null) {
            if (! empty($runningProdiIds) || ! empty($runningProdiIdsInDB)) {
                $this->toast(
                    text: 'Gagal! Tidak bisa merekap seluruh prodi karena masih ada proses rekap Program Studi yang sedang berjalan.',
                    variant: 'warning',
                    duration: 12000
                );
        ...
        if ($prId !== null) {
            ...
            if (in_array((int) $prId, $mergedRunningProdi)) {
                $this->toast(
                    text: "Gagal! Proses rekap untuk Prodi ID {$prId} sedang berjalan di latar belakang.",
                    variant: 'warning',
                    duration: 12000
                );
        ...
        if (Cache::has($cacheKey)) {
            ...
            if ($sisaWaktu > 0) {
                ...
                $this->toast(
                    text: "Fitur rekap untuk target ini sedang beristirahat setelah proses selesai. Silakan coba lagi dalam {$pesanWaktu}.",
                    variant: 'warning',
                    duration: 12000
                );
    ...
    public function generateRekapNilaiMahasiswa(?int $prId = null): void
    {
        ...
        $query->select('id')
            ->chunkById(200, function ($mahasiswas) {
                ...
                foreach ($mahasiswaIds as $mahasiswaId) {
                    ...
                    $totalSks =
                        $nilaiMahasiswa
                            ->unique('rps_id')
                            ->sum(fn ($item) => $item->rps_rel?->mk_rel?->sks_kuliah ?? 0);
    ...
    {
        // Hindari rekursif internal jika prId null, karena master loop sudah dikendalikan di generateRekapCapaianQueue
        if ($prId === null) {
            return;
        }
        ...
        NilaiMahasiswa::query()
            ->with([
                // Ambil kolom yang dibutuhkan saja untuk menghemat RAM sewaktu Eager Loading
                'mahasiswa_rel:id,pr_id',
                'rps_rel:id,is_draf',
                'rps_rel.cpmks:id,rps_id',
                'rps_rel.cpmks.scpmks:id,cpmk_id',
            ])
            ->whereHas('mahasiswa_rel', fn ($q) => $q->where('pr_id', $prId))
            ->chunkById(100, function ($nilais) {

                // Array lokal kecil, HANYA hidup selama 100 baris data chunk ini jalan
                $hasilChunk = [];
                ...
                foreach ($nilais as $nilaiMahasiswa) {
                    ...
                    foreach ($rps->cpmks as $cpmk) {
                        foreach ($cpmk->scpmks as $scpmk) {
                            ...
                            if ($nilai !== null) {
                                $hasilChunk[$mahasiswaId][$scpmk->id]['nilai'] =
                                    ($hasilChunk[$mahasiswaId][$scpmk->id]['nilai'] ?? 0) + $nilai;

                                $hasilChunk[$mahasiswaId][$scpmk->id]['jumlah'] =
                                    ($hasilChunk[$mahasiswaId][$scpmk->id]['jumlah'] ?? 0) + 1;
                            }
                ...
                // LANGSUNG eksekusi database updateOrCreate tepat setelah 100 data selesai dihitung
                foreach ($hasilChunk as $mahasiswaId => $scpmks) {
                ...
                // =========================================================================
                // MANAJEMEN SAMPAH RAM (AGRESIF)
                // =========================================================================
                // 1. Hancurkan array lokal chunk ini agar RAM kembali plong
                unset($hasilChunk, $nilais);

                // 2. Siram event listener & cache internal Eloquent agar memory tidak gembung
                NilaiMahasiswa::flushEventListeners();
                RekapSubCPMKMahasiswa::flushEventListeners();

                // 3. Paksa PHP buang sampah memori detik ini juga sebelum masuk ke 100 data berikutnya
                gc_collect_cycles();
            });
    }
